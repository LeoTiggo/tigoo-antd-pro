...
import EditPolicyFileInfoForm from '@/components/PolicyFileEdit';
          ...
          <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
            <Col md={24} sm={24}> 
              <FormItem label="公文标题" labelCol={{ span: 4}} wrapperCol={{ span: 20 }}>
                {form.getFieldDecorator('gwTitle')(<Input placeholder="请输入"/>)}
              </FormItem>
            </Col>
          </Row>
          <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
            <Col md={12} sm={24}>
              <FormItem  label="发文时间" labelCol={{ span: 8}} wrapperCol={{ span: 16 }} >
                {form.getFieldDecorator('gwTime')(<DatePicker style={{ width:'100%'}} placeholder="请选择"/>)}
              </FormItem>
            </Col>
            <Col md={12} sm={24}>
              <FormItem label="文号" labelCol={{ span: 8 }} wrapperCol={{ span: 16}}>
                {form.getFieldDecorator('gwNo')(<Input placeholder=""/>)}
              </FormItem>
            </Col>
          </Row>
          <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
...
@connect(({ policyFile, loading }) => ({
  policyFile,
  loading: loading.models.policyFile,
}))
@Form.create()
class TableList extends PureComponent {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'policyFile/fetchPolicyFiles',
    });
  }
    ...
    confirm({
      ...
      onOk() {
        ...
        return new Promise((res, rej) => {
          ...
          dispatch({
            type: 'policyFile/policyFileDelete',
            payload: { ...record },
            callback: (response) => {
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      const queryDate = fieldsValue['queryDate'];
      if(queryDate){
        startDate = queryDate[0].format('YYYY-MM-DD');
        endDate = queryDate[1].format('YYYY-MM-DD');
      };
      const values = {
        ...fieldsValue,
        startDate : startDate,
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={6} sm={24}>
            <FormItem label="文号">
              {getFieldDecorator('gwNo')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="发文时间">
      ...
      <PageHeaderWrapper >
        <Card bordered={false}>
          <div className={styles.tableList}>
            ...
            <Card bordered={true} title="政策文件" extra={<span style={{ color: "red" }}>备注：街道，县，市，省四级可用新建工作机制建设功能</span>} >
              <StandardTable
